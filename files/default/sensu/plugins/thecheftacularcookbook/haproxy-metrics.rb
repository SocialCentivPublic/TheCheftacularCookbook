...
class HAProxyMetrics < Sensu::Plugin::Metric::CLI::Graphite
  ...
  def get_stats
    ...
    if uri.is_a?(URI::Generic) && File.socket?(uri.path)
      ...
      res = `curl "#{config[:connection]}:#{config[:port]}/;csv;norefresh"`
  ...
  def run
    ...
    parsed.each do |line|
      ...
      if line[1] == 'BACKEND'
        (output "#{config[:scheme]}.#{line[0]}.session_current", line[4]) unless line[4] == ''
        (output "#{config[:scheme]}.#{line[0]}.session_total", line[7]) unless line[7] == '' 
        (output "#{config[:scheme]}.#{line[0]}.bytes_in", line[8]) unless line[8] == ''
        (output "#{config[:scheme]}.#{line[0]}.bytes_out", line[9]) unless line[9] == ''
        (output "#{config[:scheme]}.#{line[0]}.connection_errors", line[13]) unless line[13] == ''
        (output "#{config[:scheme]}.#{line[0]}.warning_retries", line[15]) unless line[15] == ''
        (output "#{config[:scheme]}.#{line[0]}.warning_redispatched", line[16]) unless line[16] == '' 
        (output "#{config[:scheme]}.#{line[0]}.response_1xx", line[39]) unless line[39] == ''
        (output "#{config[:scheme]}.#{line[0]}.response_2xx", line[40]) unless line[40] == '' 
        (output "#{config[:scheme]}.#{line[0]}.response_3xx", line[41]) unless line[41] == ''
        (output "#{config[:scheme]}.#{line[0]}.response_4xx", line[42]) unless line[42] == ''
        (output "#{config[:scheme]}.#{line[0]}.response_5xx", line[43]) unless line[43] == ''
        (output "#{config[:scheme]}.#{line[0]}.response_other", line[44]) unless line[44] == ''
        (output "#{config[:scheme]}.#{line[0]}.requests_per_second", line[46]) unless line[46] == ''
        (output "#{config[:scheme]}.#{line[0]}.requests_per_second_max", line[47]) unless line[47] == ''
        (output "#{config[:scheme]}.#{line[0]}.queue_time", line[58]) unless line[58] == ''
        (output "#{config[:scheme]}.#{line[0]}.connect_time", line[59]) unless line[59] == ''
        (output "#{config[:scheme]}.#{line[0]}.response_time", line[60]) unless line[60] == ''
        (output "#{config[:scheme]}.#{line[0]}.average_time", line[61]) unless line[61] == ''
      elsif config[:server_metrics]
        (output "#{config[:scheme]}.#{line[0]}.#{line[1]}.session_total", line[7]) unless line[7] == ''
      end
