...
#slightly modified from https://github.com/sensu/sensu-community-plugins/blob/master/plugins/haproxy/check-haproxy.rb
...
class CheckHAProxy < Sensu::Plugin::Check::CLI
  ...
  def get_services
    ...
    if uri.is_a?(URI::Generic) && File.socket?(uri.path)
      ...
      res = `curl "#{config[:stats_source]}:#{config[:port]}/;csv;norefresh"`
