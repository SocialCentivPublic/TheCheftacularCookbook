...
user_hashes.each_pair do |username, config_hash|
  ...
  node['loaded_applications'].each_key do |app_role_name|
    if authentication_bag_hash['specific_repository_authorized_keys'].has_key?(repo_hash(app_role_name)['repo_name']) && !authentication_bag_hash['specific_repository_authorized_keys'][repo_hash(app_role_name)['repo_name']].empty?
      specific_keys_arr << authentication_bag_hash['specific_repository_authorized_keys'][repo_hash(app_role_name)['repo_name']]
    end
  end
